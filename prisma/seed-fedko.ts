...
// Naruto-universe people, real Nigerian university departments
// Run: npx ts-node prisma/seed-fedko.ts
// ============================================================
...
// 10 faculties (Naruto character deans)
const FACULTIES = [
...
// Departments mapped to faculty — exactly matching your provided list
const DEPARTMENTS: Record<string, { code: string; name: string; hod: string }[]> = {
  ...
  FAG: [
    ...
    { code: "PLS", name: "Plant Science",                                 hod: "Ino Yamanaka" },
  ...
  FAL: [
    ...
    { code: "AHS", name: "Archaeology and Heritage Studies",              hod: "Hiruzen Sarutobi" },
  ...
  FEN: [
    { code: "ABE", name: "Agricultural and Bioresources Engineering",     hod: "Yamato" },
    ...
    { code: "CME", name: "Communications Engineering",                    hod: "Ibiki Morino" },
    { code: "ECE", name: "Electrical and Computer Engineering",           hod: "Tobirama Senju" },
    { code: "GLT", name: "Glass Technology",                              hod: "Baki" },
    { code: "MCE", name: "Mechanical Engineering",                        hod: "Rock Lee" },
    { code: "MTE", name: "Mechatronics Engineering",                      hod: "Neji Hyuga" },
    { code: "MET", name: "Metallurgical and Materials Engineering",       hod: "Tenten" },
    { code: "PET", name: "Petroleum Engineering",                         hod: "Rasa" },
    { code: "TST", name: "Textile Science and Technology",                hod: "Anko Mitarashi" },
    { code: "WRE", name: "Water Resources and Environmental Engineering", hod: "Suigetsu Hozuki" },
  ],
  FED: [
    { code: "ARC", name: "Architecture",                                  hod: "Sasori" },
    { code: "BLD", name: "Building",                                      hod: "Deidara" },
    { code: "FNA", name: "Fine Arts",                                     hod: "Sai" },
  ...
  FPS: [
    ...
    { code: "MCB", name: "Microbiology",                                  hod: "Orochimaru" },
  ...
  FVM: [
    ...
    { code: "VMT", name: "Veterinary Microbiology and Parasitology",      hod: "Kabuto Yakushi" },
  ],
  FPH: [
    { code: "PHA", name: "Pharmacy",                                      hod: "Sakura Haruno" },
    { code: "PHM", name: "Pharmacology and Toxicology",                   hod: "Tsunade" },
    { code: "PHC", name: "Pharmaceutical and Medicinal Chemistry",        hod: "Kabuto Yakushi" },
    { code: "PHT", name: "Pharmacognosy and Herbal Medicine",             hod: "Shizune" },
  ],
};

// ============================================================
// COURSES — real Nigerian university course titles, FUK- codes
// ============================================================
...
// ============================================================
// ROOMS — Naruto-universe venues
// ============================================================
const ROOMS = [
  { code: "AKT",  name: "Akatsuki Hall",          capacity: 500, type: "EXAM_HALL"    as RoomType, building: "Main Campus",  hasProjector: true,  hasAC: true,  hasComputers: false },
  { code: "CEA",  name: "Chunin Exam Arena",       capacity: 350, type: "EXAM_HALL"    as RoomType, building: "Exam Centre",  hasProjector: true,  hasAC: true,  hasComputers: false },
  { code: "KLHA", name: "Konoha Lecture Hall A",   capacity: 200, type: "LECTURE_HALL" as RoomType, building: "Block A",     hasProjector: true,  hasAC: true,  hasComputers: false },
  { code: "HMH",  name: "Hokage Monument Hall",    capacity: 250, type: "LECTURE_HALL" as RoomType, building: "Block B",     hasProjector: true,  hasAC: true,  hasComputers: false },
  { code: "KLHB", name: "Konoha Lecture Hall B",   capacity: 150, type: "LECTURE_HALL" as RoomType, building: "Block A",     hasProjector: true,  hasAC: true,  hasComputers: false },
  { code: "SNA",  name: "Sand Village Annex",      capacity: 100, type: "LECTURE_HALL" as RoomType, building: "Block C",     hasProjector: true,  hasAC: false, hasComputers: false },
  { code: "HCC",  name: "Hyuga Clan Classroom",    capacity: 60,  type: "CLASSROOM"    as RoomType, building: "Block D",     hasProjector: true,  hasAC: true,  hasComputers: false },
  ...
  { code: "FDL",  name: "Forest of Death Lab",     capacity: 45,  type: "LABORATORY"   as RoomType, building: "Science Blk",hasProjector: false, hasAC: true,  hasComputers: false },
  { code: "KSL",  name: "Konoha Science Lab",      capacity: 45,  type: "LABORATORY"   as RoomType, building: "Science Blk",hasProjector: false, hasAC: true,  hasComputers: false },
  { code: "HCR",  name: "Hokage Conference Room",  capacity: 30,  type: "CLASSROOM"    as RoomType, building: "Admin Blk",  hasProjector: true,  hasAC: true,  hasComputers: false },
];

// ============================================================
// STUDENTS — Naruto characters across real departments
// ============================================================
const STUDENTS = [
  // 100 Level — Boruto-generation characters
...
async function main() {
  ...
  console.log(`[2] Users: ${demoUsers.length} created (password: demo1234)`);
  ...
  const allCoursesData: {
    institutionId: string; departmentId: string; code: string;
    name: string; creditUnits: number; level: number; semester: number;
    requiresLab: boolean; maxStudents: number;
  }[] = [];

  for (const [deptCode, templates] of Object.entries(COURSES)) {
    const depId = deptMap[deptCode];
    if (!depId) { console.warn(`  WARN: dept ${deptCode} not found, skipping`); continue; }
  ...
  for (let i = 0; i < allCoursesData.length; i += 100) {
    await prisma.course.createMany({ data: allCoursesData.slice(i, i + 100) });
  }
  ...
  // 6. Lecturers — one per department (HOD as lecturer)
  ...
  // Assign lecturers to their department's courses
  ...
  for (const lec of lecturers) {
    ...
    await prisma.course.updateMany({
      where: { departmentId: dept.id, institutionId: instId },
      data: { lecturerId: lec.id },
    });
  }
  ...
  const enrollmentData: {
    ...
    status: string; semester: number; session: string;
  }[] = [];

  for (const s of STUDENTS) {
    const sid = studentMap[s.regNumber];
    if (!sid) continue;

    // Current level courses in their department
    const currentCourses = courses.filter(
      (c) => c.departmentId === deptMap[s.deptCode] && c.level === s.level && c.semester === semester
    );
    for (const c of currentCourses) {
      enrollmentData.push({ studentId: sid, courseId: c.id, status: "REGISTERED", semester, session });
    }

    // Spillover: carry-over from level below
    if (s.isSpillover && s.level >= 200) {
      ...
      for (const c of carryOverCourses) {
        const alreadyAdded = enrollmentData.find((e) => e.studentId === sid && e.courseId === c.id);
        if (!alreadyAdded) {
  ...
  for (let i = 0; i < enrollmentData.length; i += 100) {
    await prisma.studentCourse.createMany({ data: enrollmentData.slice(i, i + 100) });
  }
  ...
  const examSlotData: {
    examPeriodId: string; courseId: string; roomId: string;
    date: Date; dayOfWeek: number; slotNumber: number;
    startTime: string; endTime: string; status: string;
  }[] = [];

  const semCourses = courses.filter((c) => c.semester === semester);
  semCourses.sort((a, b) => (courseStudents[b.id]?.size || 0) - (courseStudents[a.id]?.size || 0));

  for (const course of semCourses) {
    const enrollment = courseStudents[course.id]?.size || 0;
    let roomId: string;
    if (enrollment > 350)     roomId = roomById["AKT"];
    else if (enrollment > 250) roomId = roomById["CEA"] || roomById["AKT"];
    else if (enrollment > 150) roomId = roomById["HMH"] || roomById["KLHA"];
    else if (enrollment > 100) roomId = roomById["KLHA"] || roomById["KLHB"];
    else if (enrollment > 60)  roomId = roomById["KLHB"] || roomById["SNA"];
    else if (enrollment > 30)  roomId = roomById["SNA"]  || roomById["HCC"];
    else if (course.requiresLab) roomId = roomById["SCL"] || roomById["FDL"] || roomById["KSL"];
    else                       roomId = roomById["HCR"]  || roomById["HCC"];
    ...
    for (let di = 0; di < examDates.length && !assigned; di++) {
      for (let sn = 1; sn <= 3 && !assigned; sn++) {
        ...
        for (const oid of occ) {
          ...
          if (!os) continue;
          for (const sid of myStudents) { if (os.has(sid)) { clash = true; break; } }
          if (clash) break;
        }
        if (!clash) {
    ...
    if (!assigned) console.warn(`  WARN: Could not assign slot for ${course.code}`);
  }

  for (let i = 0; i < examSlotData.length; i += 50) {
    await prisma.examSlot.createMany({ data: examSlotData.slice(i, i + 50) });
  }
  console.log(`[11] Exam Slots: ${examSlotData.length} scheduled`);

  // 12. Lecture Timetable
  const lectureTimetable = await prisma.lectureTimetable.create({
    data: {
      institutionId: instId,
      name: "Second Semester Lecture Timetable 2025/2026",
      session, semester,
  ...
  const lecSlotData: {
    lectureTimetableId: string; courseId: string; roomId: string;
    ...
    for (let day = 1; day <= 6; day++) {
      for (const time of ["08:00","10:00","12:00","14:00"]) {
        ...
        if (!clash) {
          ...
          lecSlotData.push({
            lectureTimetableId: lectureTimetable.id,
            courseId: course.id,
            roomId,
            dayOfWeek: day,
            startTime: time,
  ...
  // 13. Conflict Report (clean — zero clashes)
  await prisma.conflictReport.create({
    data: {
      examPeriodId: examPeriod.id,
      totalConflicts: 0, criticalCount: 0,
  ...
  await prisma.timetableVersion.create({
    data: {
      ...
      changes: JSON.stringify({ action: "Initial seed — corrected department structure", slots: examSlotData.length }),
    },
  });

  // 15. Notifications for all FEDKO users
  const fedkoUsers = await prisma.user.findMany({ where: { institutionId: instId }, select: { id: true } });
  await prisma.notification.createMany({
    data: fedkoUsers.map((u) => ({
      userId: u.id,
      title: "Exam Timetable Published",
      message: `The Second Semester 2025/2026 exam timetable is now available. Check your personal schedule.`,
  ...
  console.log(`\n${"=".repeat(55)}`);
  console.log(`FEDKO SEED COMPLETE in ${elapsed}s`);
  ...
  console.log("  IA  → admin@fedko.edu.ng    / demo1234");
  console.log("  TO  → officer@fedko.edu.ng  / demo1234");
  console.log("  LC  → lecturer@fedko.edu.ng / demo1234");
  console.log("  ST  → student@fedko.edu.ng  / demo1234");
}
