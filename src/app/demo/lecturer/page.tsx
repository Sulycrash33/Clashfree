...
const ACTION_CONFIG: Record<SlotAction, { label: string; color: string; bg: string; border: string; icon: any }> = {
  none:         { label: "Lecture",      color: "text-foreground/60",   bg: "bg-foreground/5",          border: "border-foreground/10",      icon: BookOpen },
...
function RankBadge({ rank }: { rank: Lecturer["rank"] }) {
  const config: Record<Lecturer["rank"], { color: string; bg: string }> = {
    ...
    "Assistant Lecturer":  { color: "text-foreground/50",    bg: "bg-foreground/5 border-foreground/10" },
    "Graduate Assistant":  { color: "text-foreground/40",    bg: "bg-foreground/5 border-foreground/10" },
  };
          ...
          {(["none","cancel","test","presentation"] as SlotAction[]).map(a => {
            ...
            return (
              <button key={a} onClick={() => setSelected(a)}
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border transition-all text-left ${selected === a ? `${cfg.bg} ${cfg.border}` : "bg-foreground/[0.03] border-foreground/10 hover:bg-foreground/5"}`}>
                <Icon className={`w-4 h-4 ${selected === a ? cfg.color : "text-foreground/30"}`} />
                <div>
                  <div className={`text-sm font-medium ${selected === a ? cfg.color : "text-foreground/60"}`}>{cfg.label}</div>
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-4 flex-wrap text-xs text-foreground/40">
        ...
        <span className="text-foreground/20">Click any slot to manage it</span>
      </div>
        ...
        <table className="w-full min-w-[540px] border-collapse">
          <thead>
            <tr className="border-b border-foreground/10 bg-foreground/[0.03]">
              <th className="px-3 py-3 text-left text-[10px] font-bold text-foreground/35 uppercase tracking-wider w-28 border-r border-foreground/10">Time</th>
              {DAYS.map(d => (
                <th key={d} className="px-2 py-3 text-center text-[11px] font-bold text-foreground/55 uppercase border-r border-foreground/10 last:border-r-0">{d.slice(0, 3)}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {TIME_BANDS.map((band, bi) => (
              <tr key={band.start} className={`border-b border-foreground/5 last:border-b-0 ${bi % 2 === 0 ? "" : "bg-foreground/[0.03]"}`}>
                <td className="px-3 py-2.5 border-r border-foreground/10 align-middle">
                  <div className="font-bold text-foreground/50 text-[10px] leading-tight whitespace-nowrap">{band.label}</div>
                </td>
                {DAYS.map(day => {
                  ...
                  if (isJumaat) {
                    return (
                      <td key={day} className="px-1.5 py-1.5 border-r border-foreground/10 last:border-r-0 align-top min-w-[90px] bg-success/5">
                  ...
                  if (cellSlots.length === 0) {
                    return (
                      <td key={day} className="px-1.5 py-1.5 border-r border-foreground/10 last:border-r-0 align-top min-w-[90px]">
                        ...
                        {cellSlots.map(slot => {
                          ...
                          return (
                            <button key={slot.id} onClick={() => onSlotClick(slot)}
                              ...
                              <div className={`text-[8px] mt-0.5 truncate flex items-center gap-0.5 ${isModified ? "text-foreground/40" : lc.text}`}>
      ...
      <div className="flex flex-wrap gap-3 text-[10px] text-foreground/35">
        {[
...
function ExamInvigilationPanel({ lecturerId }: { lecturerId: string }) {
  ...
  const roleColor: Record<InvigilationDuty["role"], string> = {
    ...
    "Relief Invigilator": "bg-foreground/10 border-foreground/10 text-foreground/40",
  };
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-xs text-foreground/35 mt-0.5">Semester 1, 2024/2025 — assigned by Examinations Officer</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={s.label} className="rounded-xl bg-foreground/5 border border-foreground/10 p-4 text-center">
            <div className={`text-2xl font-bold ${s.color}`}>{s.value}</div>
            <div className="text-[10px] text-foreground/35 mt-1">{s.label}</div>
          </div>
        ))}
      </div>

      {duties.length === 0 ? (
        <div className="rounded-2xl border border-foreground/10 p-10 text-center space-y-3">
          <Calendar className="w-8 h-8 text-foreground/15 mx-auto" />
          <div className="text-foreground/25 text-sm">No invigilation duties assigned yet.</div>
          <div className="text-foreground/15 text-xs">Duties are assigned 4 weeks before examination period.</div>
        </div>
      ) : (
        <div className="rounded-2xl border border-foreground/10 overflow-hidden">
          <div className="px-5 py-3 border-b border-foreground/10 bg-foreground/[0.03]">
            <div className="grid grid-cols-[1fr_90px_90px_100px_120px] gap-3 text-[10px] font-bold text-foreground/25 uppercase">
...
function ProfileSelector({ selected, onSelect }: { selected: Lecturer; onSelect: (l: Lecturer) => void }) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute top-full left-0 mt-2 w-72 rounded-2xl border border-foreground/15 bg-muted shadow-2xl z-40 overflow-hidden">
          <div className="px-4 py-2 border-b border-foreground/10">
            <p className="text-xs text-foreground/30 font-medium">Select Lecturer Profile</p>
          </div>
          <div className="py-1">
          ...
          <div className="p-3 rounded-xl bg-foreground/5 border border-foreground/10 space-y-2">
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {["Timetable", "Profile", "Exam"].map(tab => (
                <button key={tab}
                  className={`text-xs px-2 py-1.5 rounded-lg border transition-colors ${tab === "Timetable" ? "bg-success/15 border-success/30 text-success" : "bg-foreground/5 border-foreground/10 text-foreground/40 hover:bg-foreground/10"}`}>
                  {tab}
                </button>
              ))}
            </div>
          </div>
          <p className="text-[10px] text-foreground/25 text-center">Settings applied to your ScheduleFlex lecturer profile.</p>
        </div>
      </div>
    </div>
  );
      ...
      <div className="px-4 sm:px-6 py-8 space-y-6 max-w-5xl mx-auto">
        ...
        <div className="rounded-2xl border border-foreground/10 bg-foreground/[0.03] p-5">
          <div className="flex flex-col sm:flex-row items-start gap-5">
            <div className="flex flex-col items-center gap-1.5">
              ...
              <span className="text-[9px] text-foreground/25">Tap to upload photo</span>
            </div>
              ...
              <div className="flex items-start justify-between gap-3 flex-wrap">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    ...
                    <span className="text-foreground/40 text-xs">{lecturer.deptName}</span>
                  </div>
                  <p className="text-foreground/25 text-xs mt-0.5">{lecturer.staffId}</p>
                </div>
                <div className="flex items-center gap-2">
        ...
        {(cancelledCount + testCount + presentationCount) > 0 && (
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <span className="text-xs text-foreground/25 ml-1">Students notified via WhatsApp</span>
          </div>
        )}
        ...
        {activeTab === "timetable" && (
          <div className="space-y-5">
            <div className="rounded-2xl border border-foreground/10 overflow-hidden">
              <div className="px-5 py-3 border-b border-foreground/10 bg-foreground/[0.03]">
                <p className="text-sm font-semibold text-foreground/60">Assigned Courses — Weekly Slot Distribution</p>
              </div>
              <div className="divide-y divide-white/5">
              ...
              <div className="px-5 py-3 border-t border-foreground/10 bg-foreground/[0.03] flex items-center justify-between">
                <span className="text-xs text-foreground/35">{lecturer.coursesCurrent.length} courses · {mySlots.length} total slots</span>
                <span className="text-sm font-bold text-success">{totalHours} hrs/week</span>
              </div>
            </div>

            {mySlots.length === 0 ? (
              <div className="rounded-2xl border border-foreground/10 p-12 text-center space-y-2">
                <CalendarClock className="w-8 h-8 text-foreground/20 mx-auto" />
                <p className="text-foreground/30 text-sm">No timetable slots for this lecturer in the demo data.</p>
                <p className="text-foreground/20 text-xs">Courses: {lecturer.coursesCurrent.join(", ")}</p>
              </div>
            ) : (
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <button onClick={() => handleDownload("print")}
                className="flex items-center gap-2 px-4 py-2 rounded-xl bg-foreground/5 border border-foreground/10 text-foreground/50 text-sm hover:bg-foreground/10 transition-colors">
        ...
        {activeTab === "profile" && (
          <div className="space-y-5">
            <div className="rounded-2xl border border-foreground/10 bg-foreground/[0.03] p-6 space-y-5">
              <div className="flex items-start gap-5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-foreground/35 text-xs mt-1">{lecturer.specialization}</p>
                </div>
              </div>
              {lecturer.bio && (
                <p className="text-sm text-foreground/55 leading-relaxed border-t border-foreground/10 pt-4">{lecturer.bio}</p>
              )}
            </div>
              ...
              <div className="rounded-2xl border border-foreground/10 p-5 space-y-3">
                ...
                <ul className="space-y-2">
                  {lecturer.qualifications.map(q => (
                    <li key={q} className="flex items-start gap-2 text-sm text-foreground/60">
            ...
            <div className="rounded-2xl border border-foreground/10 p-5 space-y-3">
              <h3 className="text-sm font-semibold text-foreground/50">Contact and Office</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {[
                  ...
                  <div key={c.label} className="rounded-xl bg-foreground/5 border border-foreground/10 p-3">
                    <div className="text-xs text-foreground/35">{c.label}</div>
                    <div className="text-sm text-foreground/70 mt-0.5 truncate">{c.value}</div>
                  </div>
                ))}
              </div>
            </div>
